...
from .error import HalogenError
...
class HalogenConfig():
	...
	def get_sub_config(self, path: str):
		...
		return HalogenConfig(self._os, self._directory, cfg, self._dev)



class HalogenConfigLoader():
	...
	def make_parser(self):
		...
		self.parser = argparse.ArgumentParser(
			prog = "halogen",
			description = "Boots up Halogen. See --help for help."
		)
	...
	def init_default_config(self):
		...
		if self.os == "windows":
			self.directory = Path(os.environ["APPDATA"]) / "halogen"
		elif self.os == "linux":
			self.directory = Path(os.path.expanduser("~/.config/halogen"))
		else:
			raise HalogenError(f"Running in unsupported operating system: {OS}.")
	...
	def load(self):
		...
		if not self.config_file.exists():
			raise HalogenError(f"Config file '{self.config_file}' does not exist! Cannot start halogen.")

		with open(self.config_file, 'rb') as file:
			try:
				data = tomllib.load(file)
			except tomllib.TOMLDecodeError:
				raise HalogenError(f"Invalid config file '{self.config_file}'.")
